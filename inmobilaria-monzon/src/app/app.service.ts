...
export class AppService {
  ...
  public getProperties(): Observable<Property[]> {
    return this.requestService.get("obtener_inmuebles.php").pipe(
      map((data: any) => data as Property[])
  ...
  public getPropertyById(id: number): Observable<Property> {
    return this.requestService.get(`obtener_inmueble_por_id.php?id=${id}`).pipe(
      map((data: any) => data as Property)
    );
  }
  ...
  public getFeaturedProperties(): Observable<Property[]>{
    return this.requestService.get("obtener_propiedades_destacadas.php").pipe(
      map((data: any) => data as Property[])
    );
  }
  
  public getRelatedProperties(): Observable<Property[]> {
    return this.requestService.get(`obtener_propiedades_relacionadas.php`).pipe(
      map((data: any) => data as Property[])
    );
  }
  ...
  public getLocations(): Observable<Location[]> {
    return this.requestService.get('obtener_ubicaciones.php').pipe(
      map((data: any) => data as Location[])
    );
  }
  ...
  getFullAddress(lat: number, lng: number): Observable<string> {
    return this.http.get('https://maps.googleapis.com/maps/api/geocode/json?latlng=' + lat + ',' + lng + '&key=' + this.apiKey)
      .pipe(map((data: any) => data.results[0].formatted_address));
  }
  ...
  public getFeatures(){
    return [ 
      ...
      { id: 5, name: 'Heladera', selected: false },
      ...
      { id: 12, name: 'Jacuzzi', selected: false },
    ]
  }
