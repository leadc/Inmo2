...
class BORMObject
{
     ...
     */
    protected function SQLSelect($filters = [], $order = [], $join = [], $joinfields = [])
    {
        // Set the table 
     ...
     */
    protected function SQLJoin($filters = [], $order = [], $join = [], $joinFields = [])
    {
        // Creates a select statement
        $sql = $this->SQLSelect($filters, $order, $join, $joinFields);
        //Log::WriteLog(MAIN_LOG, [get_class($this)." SQL JOIN: $sql"]);
        // Executes the query
        $connectionClassName = self::$ORMConnections . $this->mapping->dbConnectionClass;
        $resultado = $connectionClassName::Query($sql);
        // Returns the results
    ...
    protected function InsertSQL()
    {
        ...
        Log::WriteLog(MAIN_LOG, ["sql:" . $sql]);
        //Log::WriteLog(MAIN_LOG, [get_class($this)." SQL INSERT: $sql"]);
        // Executes the statement
        $connectionClassName = self::$ORMConnections . $this->mapping->dbConnectionClass;
        Log::WriteLog(MAIN_LOG, ["sqlcon:" . $connectionClassName]);
